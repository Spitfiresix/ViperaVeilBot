...
from src.dtos.playerdtos import PlayerDto
...
def convert_player_to_dto(player):
    dto = PlayerDto(player.name, player.id_user)
    dto.userId = player.id_user
    dto.wins = player.wins
    dto.losses = player.losses
    dto.matchesplayed = player.nb_matches
    dto.winloassratio = player.wlr
    dto.elo = player.elo
    dto.highestelo = player.elo  # Assuming highest elo is the same as current elo
    dto.longestwinstreak = player.most_wins_in_a_row
    dto.longestlossstreak = player.most_losses_in_a_row
    dto.currentwinstreak = player.current_win_streak
    dto.currentlossstreak = player.current_lose_streak
    dto.SelectedModes = player.SelectedModes
    return dto
