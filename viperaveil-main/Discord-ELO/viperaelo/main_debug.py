...
# test guild
DISCORD_MAIN_GUILD_ID = os.environ.get('DEV_TESTDISCORD')
# real guild
#DISCORD_MAIN_GUILD_ID = 1102750893822197802
...
async def on_ready():
    ...
    for guild in BOT.guilds:
        ...
        game = get_games(API_ENDPOINT, guild.id)
        if game is not None:
            assign_game(guild.id, game)
            print(f"Game {guild.id} loaded")
        else:
            # create new game
             response = create_game(API_ENDPOINT, guild.id, Game(guild.id))
             game = get_games(API_ENDPOINT, guild.id) 
             assign_game(API_ENDPOINT, guild.id, game)
...
async def on_command_completion(ctx):
    ...
    update_game(API_ENDPOINT, ctx.guild.id, GAMES[ctx.guild.id])
